...
export default function ProductosListScreen({ navigation }: any) {
  const { data, loading, error, retry } = useFetch<Producto[]>(
    "https://miapi.com/productos"
  );
  ...
  return (
    <FlatList
      ...
      renderItem={({ item }) => (
        ...
        >
          <Text style={{ color: colors.primary, fontWeight: "bold" }}>{item.nombre}</Text>
          <Text style={{ color: colors.text }}>Cantidad: {item.cantidad}</Text>
          <Text style={{ color: colors.subtle }}>{item.descripcion}</Text>
        </Pressable>
      )}
    />
  );
